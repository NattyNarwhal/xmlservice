...
$pipes = [];
// using an array here will require PHP 7.4+
$proc = proc_open([$xmlserviceCli, "-c", $ctl, "-i", $ipc], $pipe_desc, $pipes);
if ($proc) {
	fwrite($pipes[0], $clobIn);
	fclose($pipes[0]);
	$clobOut = stream_get_contents($pipes[1]);
	fclose($pipes[1]);
	fclose($pipes[2]);
	$ret = proc_close($proc);
	// who cares
